...
u8 A9800ReadReg(u8 addr){
  ...
  SPI_A9800_CS_HIGH();
  delay_us(30); //  tSRW/tSRR (=20us) minus tSCLK-NCS
...
void A9800WriteReg(u8 addr, u8 data){
  ...
  delay_us(30); // tSCLK-NCS for write operation
  SPI_A9800_CS_HIGH();
  delay_us(150); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound
}
...
// wait for more than one frame period
	delay_us(300); // assume that the frame rate is as low as 100fps... even if it should never be that low
...
s16 A9800_getMotion(void)
{
	...
	motion = A9800ReadReg(SPI_A9800_REG_Delta_Y_L);
	motion = A9800ReadReg(SPI_A9800_REG_Delta_Y_H)<<8 | motion;
	return -motion;
}
float A9800_getSpeed(void){
	float speed = .0;
	speed = (float)A9800_getMotion()/650.0;
	if(speed>5)speed = 0;
	return speed;
}
void A9800_PowerUp(void)
{
  ...
  A9800UploadFiremware((u8*)firmwareA4, firmwareA4Length);
